...
// void main() {
//   vec2 p = (gl_FragCoord.xy - 0.5 * iResolution.xy) /  iResolution.x; // (-0.5~0.5, -0.5~0.5)

//   float len = dot(p, p) * 2. - 0.5; // (0~0.5, 0~0.5)

//   // gl_FragColor = vec4(vec3(p.x + 0.5), 1.);
//   // float val = 0.5 * sin(len * 10.);
//   // float val = len + 0.5;
//   // float val = p.x * p.x;
